package com.coderlong.bigdata.sessionproject.test;
...
//        }
    ...
    public static void mockDataTest(JavaSparkContext sc, SQLContext sqlContext){
        List<Row> rows = new ArrayList<>();
        Random random = new Random();
        String[] userNameArr = {"zs", "ls", "ww", "oo", "ii"};
        for (int i = 0; i < 10; i++) {
            Integer userId = random.nextInt(100);
            String userName = userNameArr[random.nextInt(4)];
            Integer age = random.nextInt(40);

            Row row = RowFactory.create(userId, userName, age);
            rows.add(row);
        ...
        Dataset<Row> dataset = sqlContext.createDataFrame(rowJavaRDD, schema);
        System.out.println("dataset show====================================================");
        dataset.show();
        Row[] rs = (Row[])dataset.take(1);
        for (Row r : rs) {
            System.out.println(r);
        }

        dataset.registerTempTable("tmp_user");

        Dataset<Row> dsBySql = sqlContext.sql("select * from tmp_user where userId < 50");
        System.out.println("dsBySql show====================================================");
        dsBySql.show();
